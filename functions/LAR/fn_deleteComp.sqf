...
}forEach ( _compReference call LAR_fnc_getCompObjects );
...
{
	if ( !isNil "_x" && { _x select 0 == _compReference } ) exitWith { LAR_spawnedCompositions set [ _forEachIndex, nil ] };
}forEach LAR_spawnedCompositions;

if ( { !isNil "_x" }count LAR_spawnedCompositions isEqualTo 0 ) then {
