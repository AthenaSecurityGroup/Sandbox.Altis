/*
	...
	This is the main function responsible for compositions.
	
	EXEC TYPE:	call
	INPUTS:
	0	STRING	Composition class name from (missionConfigFile cfgCompositions).
	1	ARRAY	Deployment point (ATL). Accepts: ARRAY, OBJECT, MARKER, LOCATION.
	2	ARRAY	Translate [X,Y,Z] from deployment point.
	3	SCALAR	Direction to face the composition. Use TRUE for dir of OBJECT, MARKER, LOCATION from [1].
	4	BOOL	Align to surface normal.
	OUTPUT:
	STRING	_COMPOSITION_NAME_ + _## - ex: myComposition_00
...
private _fnc_setPositionAndRotation = {
	private[ "_pos", "_newPosX", "_newPosY", "_newPosASL", "_newPosZ", "_rotation", "_mkrPos", "_cfg"];
	params[
		[ "_obj", objNull ],
		[ "_cfgOffset", [0,0,0] ],
		[ "_cfgRot", [0,0,0] ],
...
//		};
	...
	if !( isNull _obj ) then {
		...
		// diag_log format ["_posWorld:			%1", _pos];
		// diag_log format ["_ATL(2):				%1", (getPosATL _obj select 2)];
		...
		//	SIMLULATION CHECK -- ADDED, DIFFUSION9 2017-04-07
		_disableSimulation = getNumber (_cfg >> "Attributes" >> "disableSimulation");
		if (_disableSimulation == 1) then {_obj enableSimulationGlobal false} else {_obj enableSimulationGlobal true};
	};
	_pos
};
...
private _fnc_spawnObject = {
	...
	if ( random 1 <= _presence && { call compile _preCondition } ) then {
		...
		//	PASS _cfg TO FUNCTION TO GET CONFIG DATA -- ADDED, DIFFUSION9 2017-04-07
		_position = [ _veh, _position, _rotation, _ATLOffset, _randomStartPos, _needsSurfaceUP, _placementRadius, _cfg] call _fnc_setPositionAndRotation;
